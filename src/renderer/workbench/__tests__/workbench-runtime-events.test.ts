...
describe("Workbench runtime event loading", () => {
  it("merges live runtime events that arrive while a snapshot is loading", () => {
    ...
    expect(source).toContain("...prev.filter(event => event.threadId === snap.activeThreadId)")
    expect(source).toContain("...pendingRuntimeEvents.current.filter(event => event.threadId === snap.activeThreadId)")
  ...
  it("captures live events for the thread currently being loaded or selected", () => {
    ...
    expect(source).toContain("loadingThreadIdRef.current = snap.activeThreadId")
    expect(source).toContain("loadingThreadIdRef.current = threadId")
    expect(source).toContain("event.threadId === activeThreadId || event.threadId === loadingThreadIdRef.current")
    expect(source).toContain("if (loadingThreadIdRef.current === threadId) loadingThreadIdRef.current = null")
  })
})
