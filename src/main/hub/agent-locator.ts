...
function fromPath(cmd: string): string | null {
  try {
    ...
    const first = process.platform === 'win32'
      ? paths.find(path => /\.(cmd|exe)$/i.test(path)) || paths[0]
      : paths[0]
    return first || null
  } catch {
    return null
...
function terminalCliCandidates(envVar: string, names: string[]): AgentBinaryCandidate[] {
  const cands: Array<AgentBinaryCandidate | null> = [envCandidate(envVar)]
  for (const n of names) {
    cands.push(npmCandidate(n))
    cands.push({ source: 'terminal', label: '终端版', path: join(homedir(), '.local', 'bin', n + '.exe') })
...
/* ---------------- Conservative external candidates ---------------- */

export function geminiCandidates(): AgentBinaryCandidate[] {
  return terminalCliCandidates('GEMINI_CLI_PATH', ['gemini'])
}

export function locateGeminiBinary(): string | null {
  return geminiCandidates()[0]?.path ?? null
}

export function codeBuddyCandidates(): AgentBinaryCandidate[] {
  return terminalCliCandidates('CODEBUDDY_PATH', ['codebuddy', 'CodeBuddy'])
}

export function antigravityCandidates(): AgentBinaryCandidate[] {
  return manualCandidates(dedupe([
    ...terminalCliCandidates('ANTIGRAVITY_PATH', ['antigravity', 'Antigravity']),
    ...terminalCliCandidates('FANZHONGLI_PATH', ['antigravity', 'Antigravity', '反重力'])
  ]), 'Only terminal CLI candidates are shown. Add non-interactive args before dispatch.')
}

export function mimocodeCandidates(): AgentBinaryCandidate[] {
  return terminalCliCandidates('MIMOCODE_PATH', ['mimocode', 'Mimocode'])
}

export function zcodeCandidates(): AgentBinaryCandidate[] {
  return terminalCliCandidates('ZCODE_PATH', ['zcode', 'ZCode'])
}

export function reasonixCandidates(): AgentBinaryCandidate[] {
  return terminalCliCandidates('REASONIX_PATH', ['reasonix', 'Reasonix'])
}
...
export function locateAgentCandidates(): Record<string, AgentBinaryCandidate[]> {
  return {
    ...
    codebuddy: codeBuddyCandidates(),
    antigravity: antigravityCandidates(),
    mimocode: mimocodeCandidates(),
    zcode: zcodeCandidates(),
    reasonix: reasonixCandidates()
  }
}
