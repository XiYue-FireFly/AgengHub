...
- **GitHub**：`origin` = https://github.com/XiYue-FireFly/AgengHub.git（主仓库，默认分支 main）。
- **gh CLI**：`C:\Program Files\GitHub CLI\gh.exe`，使用具备 `XiYue-FireFly/AgengHub` 推送权限的账号登录，token scopes 含 `repo, workflow`。
...
### 8.2 新电脑冷启动
1. **拿到仓库访问**：仓库地址为 `XiYue-FireFly/AgengHub`。新机使用有访问权限的 GitHub 账号 `gh auth login` 后 clone。`git clone https://github.com/XiYue-FireFly/AgengHub.git`（路径自定，本文 §1 的 `C:\Users\Admin\...` 是旧机路径，新机用自己的）。
